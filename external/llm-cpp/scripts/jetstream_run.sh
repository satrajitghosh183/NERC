#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════
# jetstream_run.sh — One-shot setup, benchmark, and evaluation on H100
#
# Clone this repo, checkout Experimental-cpp-llm, then:
#   chmod +x scripts/jetstream_run.sh
#   ./scripts/jetstream_run.sh
#
# What it does:
#   1. Verifies prerequisites (CUDA, cmake, python3, git)
#   2. Clones cpp-llm-clean alongside this repo
#   3. Builds both C++ projects with CUDA + H100 optimizations
#   4. Sets up Python venv with PyTorch+CUDA and OLMo-core
#   5. Downloads GPT-2 tokenizer + TinyStories data
#   6. Runs 3-way training benchmark (30M, 125M, 350M)
#   7. Trains models for generation quality evaluation
#   8. Runs 10-prompt chat evaluation with automated grading
#   9. Produces final report in results/
# ═══════════════════════════════════════════════════════════════════════
set -euo pipefail
...
CPP_LLM_DIR="${CPP_LLM_DIR:-$REPO_ROOT/../cpp-llm-clean}"
CPP_LLM_REPO="${CPP_LLM_REPO:-git@github.com:RU-ECE/cpp-llm-clean.git}"
...
# Training for chat quality (longer)
...
check_prerequisites() {
    ...
    if python3 -c "import torch; assert torch.cuda.is_available()" 2>/dev/null; then
        ...
        warn "PyTorch CUDA check failed (will be set up in venv)"
    fi
...
# ── 2. Clone cpp-llm-clean ──
clone_cpp_llm() {
    section "Cloning cpp-llm-clean"

    if [ -d "$CPP_LLM_DIR" ]; then
        log "cpp-llm-clean already exists: $CPP_LLM_DIR"
        cd "$CPP_LLM_DIR" && git pull --ff-only 2>/dev/null || true
        cd "$REPO_ROOT"
    else
        log "Cloning $CPP_LLM_REPO -> $CPP_LLM_DIR"
        git clone "$CPP_LLM_REPO" "$CPP_LLM_DIR"
    fi
}

# ── 3. Build llm-cpp (this repo — Experimental) ──
build_llm_cpp() {
...
# ── 4. Build cpp-llm-clean ──
build_cpp_llm() {
    section "Building cpp-llm-clean"

    if [ ! -d "$CPP_LLM_DIR" ]; then
        warn "cpp-llm-clean not found, skipping"
        return
    fi
    ...
    # Find the training binary (could be train, olmo_train, or main)
    CPP_LLM_TRAIN=""
...
setup_python_env() {
    ...
    if [ -d "$VENV_DIR" ] && "$VENV_DIR/bin/python" -c "import torch" 2>/dev/null; then
        ...
        "$VENV_DIR/bin/pip" install ai2-olmo-core -q \
            || warn "OLMo-core install failed (generation eval will use fallback)"

        "$VENV_DIR/bin/pip" install numpy -q
    fi

    # Verify CUDA works in the venv
    if "$VENV_DIR/bin/python" -c "import torch; assert torch.cuda.is_available()" 2>/dev/null; then
        log "PyTorch CUDA verified in venv"
    else
        warn "PyTorch CUDA not available in venv — Python benchmarks may be slow"
    fi
}
...
prepare_data() {
    ...
    if [ -f "$DATA_DIR/tinystories_gpt2.npy" ]; then
        ...
        if [ -f "$REPO_ROOT/build/prepare_data" ]; then
            ...
            # Fallback: generate random tokens for benchmarking
...
# ── 7. Generate JSON configs for chat binary ──
...
run_training_benchmark() {
    ...
    # ── 350M benchmark (manual — not in benchmark.py) ──
    log "Benchmarking 350M model (llm-cpp only)..."

    # Create a temporary .conf with bench settings
...
CONFEOF
    ...
    # 350M with cpp-llm (if binary found)
...
# ── 9. Train Models for Chat Evaluation ──
train_for_chat() {
    section "Training models for chat evaluation"

    mkdir -p "$CHECKPOINT_DIR"

    # Train 30M llm-cpp (non-fused, for chat binary compatibility)
...
# ── 10. Run Chat Evaluation ──
run_chat_evaluation() {
    section "Running chat evaluation (10 prompts)"

    # Generate JSON configs needed by chat binary
    generate_json_configs
    ...
    if [ -f "$CHECKPOINT_DIR/30M_cpplm.pt" ] && [ -n "${CPP_LLM_CHAT:-}" ] && [ -f "${CPP_LLM_CHAT:-}" ]; then
        # cpp-llm might use different config format — try finding its config
        CPP_LLM_CFG=""
...
HEADER
    ...
    if [ -f "$RESULTS_DIR/bench_350M_experimental.log" ]; then
        grep -E '(tok/s|params|Backend|Model)' "$RESULTS_DIR/bench_350M_experimental.log" >> "$REPORT" 2>/dev/null || true
    fi
